...
interface FormState { id?: string; label: string; color: string; sort_order: number; }
...
export default function MasterTab({ category, singular }: { category: string; singular: string }) {
  ...
  const [form, setForm] = useState<FormState>({ label: '', color: PRESET_COLORS[0], sort_order: 0 });
  ...
  const openNew = () => { setForm({ label: '', color: PRESET_COLORS[0], sort_order: (items[items.length - 1]?.sort_order ?? 0) + 1 }); setEditing(false); setError(''); setModalOpen(true); };
  const openEdit = (m: MasterItem) => { setForm({ id: m.id, label: m.label, color: m.color || PRESET_COLORS[0], sort_order: m.sort_order }); setEditing(true); setError(''); setModalOpen(true); };
  ...
  const submit = async () => {
    ...
    const payload = { ...(form.id ? { id: form.id } : {}), category, label: form.label.trim(), color: form.color, sort_order: order, is_active: true };
    try {
